local sounds = {}
	...
	function sounds.setMusic(file_path)
		if sounds.music.file_path == file_path then
			...
			if sounds.music.resourse then
				sounds.music.resourse:stop()
			end
			sounds.music.file_path = file_path
			sounds.music.resourse = love.audio.newSource(file_path,"static")
			sounds.music.resourse:setLooping(true)
			sounds.music.resourse:setVolume(settings.window.music_vol * 0.01)
			sounds.music.resourse:play()
		end
	end
	...
	function sounds.load(file_path)
		...
		local sound = {
			resourse = love.audio.newSource(file_path,"static"),
			path = file_path
		}
	...
	function sounds.play(sound)
		sound.resourse:setVolume(settings.window.sound_vol * 0.01)
		sound.resourse:play()
	end
