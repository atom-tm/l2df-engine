local core = l2df
assert(type(core) == "table" and core.version >= 1.0, "Battle works only with l2df v1.0 and higher")

local battle = { }

	battle.res = core.import "battle.resources"
	battle.graphic = core.import "battle.graphic"
	battle.entities = core.import "battle.entities"
	battle.control = core.import "battle.control"
	battle.physix = core.import "battle.physix"
	battle.collision = core.import "battle.collision"
	battle.ai = core.import "battle.ai"
	...
	function battle:createOptimizedLists()
		...
		self.objs = {}
		self.effs = {}
		for key in pairs(self.entities.list) do
			if self.entities.list[key].head.type == "character" then
				table.insert(self.chars,self.entities.list[key])
			elseif self.entities.list[key].head.type == "object" then
				table.insert(self.objs, self.entities.list[key])
			end
		end
		for key in pairs(self.entities.effects) do
			table.insert(self.effs,self.entities.effects[key])
		end
		self.objects = #self.chars + #self.objs + #self.effs
	end
